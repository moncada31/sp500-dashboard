...
def obtener_datos_ticker(ticker: str, sector: str) -> dict:
    """
    Descarga métricas financieras clave de un ticker usando yfinance.
    Devuelve un dict con: ticker, sector, precio, trailing PER,
    forward PER, PEG ratio y valoración (semáforo).
    """
    try:
...
with st.sidebar:
    ...
    filtros_val = st.multiselect(
        "Mostrar solo:",
        options=["🟢 Compra Potencial", "🟡 Mantenimiento", "🔴 Alerta Corrección"],
        default=["🟢 Compra Potencial", "🟡 Mantenimiento", "🔴 Alerta Corrección"],
    )
