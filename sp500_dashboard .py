...
st.set_page_config(
    ...
    page_icon="📊",
    layout="wide",
    initial_sidebar_state="collapsed",   # en móvil ahorra espacio; en escritorio
                                         # el usuario lo abre con un clic
)

# CSS personalizado: semáforo, estilo general y optimización móvil
st.markdown("""
<style>
    /* ── ESCRITORIO: estilos base ───────────────────────────── */
    ...
    /* ── TABLA: scroll horizontal táctil en cualquier pantalla ─ */
    [data-testid="stDataFrame"] > div {
        overflow-x: auto !important;
        -webkit-overflow-scrolling: touch !important;
    }

    /* ── MÓVIL: breakpoint 768 px ───────────────────────────── */
    @media (max-width: 768px) {

        /* Menos padding lateral para aprovechar la pantalla */
        .block-container {
        ...
        /* ── Columnas: wrap automático en 2 por fila ─────────── */
        /* Las columnas de st.columns() usan flexbox con estilos
           inline; necesitamos !important para sobreescribirlos.  */
        ...
        /* ── Métricas: texto más grande y legible ─────────────── */
        ...
        /* ── Botones: área de toque mínima de 48 px (WCAG) ────── */
        .stButton > button {
        ...
        /* ── Selectbox y multiselect más fáciles de pulsar ─────── */
        [data-baseweb="select"] > div:first-child {
            min-height: 46px !important;
            font-size: 0.9rem !important;
        }

        /* ── Tabla: asegurar scroll horizontal con dedo ─────────  */
        ...
        /* ── Sidebar: ícono hamburguesa más fácil de tocar ──────── */
        [data-testid="collapsedControl"] {
            width: 48px !important;
            height: 48px !important;
        }

        /* ── Bloque Pine Script: fuente legible en pantalla chica ─ */
        .pine-block { font-size: 0.72rem !important; }
    }
</style>
...
# Fila 1 de KPIs: Total · Compra · Mantenimiento
# Fila 2 de KPIs: Alerta · Sin Datos
# En escritorio ambas filas quedan alineadas; en móvil cada columna
# ocupa ~50% del ancho gracias a la media query (2 por fila).
kpi1, kpi2, kpi3 = st.columns(3)
kpi4, kpi5       = st.columns(2)

label_empresas = "🏢 Total S&P 500" if modo_todas else "🏢 Empresas"
kpi1.metric(label_empresas,          len(df))
kpi2.metric("✅ Compra Potencial",   len(df[df["Valoración"].str.contains("Compra",  na=False)]))
kpi3.metric("⚠️ Mantenimiento",      len(df[df["Valoración"].str.contains("Manten",  na=False)]))
kpi4.metric("🚨 Alerta Corrección", len(df[df["Valoración"].str.contains("Alerta",  na=False)]))
kpi5.metric("⚪ Sin Datos",          n_sin_datos)
...
if ticker_sel:
    ...
    # Fila 1: Precio · PER · Forward PER
    # Fila 2: PEG · Valoración
    # Con la media query de móvil cada columna ocupa ~50%:
    # fila 1 → 2 + 1  |  fila 2 → 1 + 1
    d1, d2, d3 = st.columns(3)
    d4, d5     = st.columns(2)

    d1.metric("💵 Precio",
              f"${fila['Precio ($)']:,.2f}" if pd.notna(fila["Precio ($)"]) else "N/A")
    d2.metric("📈 PER (TTM)",
              f"{fila['PER']:.1f}" if pd.notna(fila["PER"]) else "N/A",
              delta=f"Ref sector: {per_ref_ticker:.0f}",
              delta_color="inverse" if pd.notna(fila["PER"]) and fila["PER"] > per_ref_ticker else "normal")
    d3.metric("🔭 Forward PER",
              f"{fila['Forward PER']:.1f}" if pd.notna(fila["Forward PER"]) else "N/A")
    d4.metric("📐 PEG Ratio",
              f"{fila['PEG']:.2f}" if pd.notna(fila["PEG"]) else "N/A",
    ...
    d5.metric("🚦 Valoración", fila["Valoración"])
